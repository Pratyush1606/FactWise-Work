...
class hello(APIView):
    ...
    def get(self, request, user_id=""):
        # x = UserModel.objects.get(username="matt")
        # print(request.user, request.session)
        data = {
            "query": "Hello World",
            "comments": "Request By an Anonymous User" 
        }
        return Response(data=data, status=status.HTTP_200_OK)

    def post(self, request, user_id):
        # using pre-used user and enterprise for testing
        ...
        x = UserAuth.objects.create(**data)
        data = {
            "username": x.username,
            "refresh": str(RefreshToken.for_user(x)),
            "access": str(AccessToken.for_user(x))
        }
        return Response(data=data, status=status.HTTP_201_CREATED)

class hello1(APIView):
    ...
    def get(self, request):
        ...
        data = {
            "query": "Hello World",
            "comment": "Requested By an Authenticated User, " + str(request.user)
        }
        return Response(data=data, status=status.HTTP_200_OK)
...
class user_auth(APIView):
    ...
    def post(self, request, user_id):
        ...
        # Method to post username and password before creating a user
        try:
            username = request.data["username"]
        ...
        try:
            ...
            # When a same username exists
            return Response(data="Username exists!", status=status.HTTP_409_CONFLICT)
        
        data_to_return = {
            "username": user_auth.username,
            "refresh": str(RefreshToken.for_user(user_auth)),
            "access": str(AccessToken.for_user(user_auth))
        }
