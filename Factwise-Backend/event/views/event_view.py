...
from event.views.permissions_check_functions import module_permission_check
...
class event_permission(APIView):
    ...
    def get(self, request, event_id):
        ...
        event_permissions = EventPermission.objects.filter(event_id=event_id)
        serializers = EventPermissionSerializer(event_permissions, many=True)
...
class event(APIView):
    ...
    def get(self, request, event_id):
        ...
        allowed_action_group_list = ["buyer_event", "seller"]
        ...
        # Since this api method can be used by both buyer and seller side, so getting all
        # the entity id list for this event to check the permissions of this method for
        # this user_id and every entity_id and if the user_id is allowed from any entity_id,
        # then user can access this api
        # Also while checking the allowed action group list, if the passed entity_id is
        # buyer then the while checking the userpermissions for this user_id and entity_id
        # will only contain the buyer side permissions, so any seller permission in the 
        # allowed_action_group_list won't affect that check and same goes for seller side.

        seller_id_list = list(EventItemSeller.objects.filter(event_id=event_id).values_list("seller_id", flat=True))
        buyer_id = event_info.buyer_id_id
        all_entity_list = seller_id_list + [buyer_id]

        is_permitted = False
        for entity_id in all_entity_list:
            is_permitted = is_permitted or module_permission_check(event_id, "event", user_id, allowed_action_group_list, entity_id)
            if(is_permitted):
                # One check found where user has got permission, 
                # then no need to further check
                break
    ...
    def patch(self, request, event_id):
        ...
        allowed_action_group_list = ["buyer_event"]
        ...
        buyer_id = event.buyer_id_id
        all_entity_list = [buyer_id]
...
class event_seller_status_change(APIView):
    ...
    def patch(self, request, event_id, seller_id):
        ...
        allowed_action_group_list = ["seller"]
        ...
        seller_id_list = list(EventItemSeller.objects.filter(event_id=event_id).values_list("seller_id", flat=True))
        all_entity_list = seller_id_list
...
class shift_event_draft_event(APIView):
    ...
    def post(self, request, event_id):
        ...
        allowed_action_group_list = ["buyer_event_creation"]
