...
class LatestBidViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db
        '''
    ...
    def test_get_valid_lastest_bid(self):
        '''
        Getting bid with event id 1 and seller id 2
        '''
        response = self.client.get(reverse("event:lastest_bid" , args=[1,2]))
    ...
    def test_get_invalid_lastest_bid(self):
        '''
        Getting bid with event id 1 and seller id 3, which does not exist
        '''
        response = self.client.get(reverse("event:lastest_bid" , args=[1,3]))
        self.assertEqual(response.status_code, 404)
        '''
        Getting no latest bid
        '''
