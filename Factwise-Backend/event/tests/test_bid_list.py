...
class BidListViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db
        '''
    ...
    def test_get_valid_bid_list(self):
        data = {
            "bid_info":
            {
        ...
        response = self.client.post(reverse("event:post_bid", args=[1]), json.dumps(data), content_type='application/json')
        '''
        Getting bid with event_id  1
        '''
        response = self.client.get(reverse("event:post_bid" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.data), 2)
        self.assertEqual(response.data[0]["bid_info"]["bid_id"], 1)
        self.assertEqual(response.data[1]["bid_info"]["bid_id"], 2)
        ...
        response = self.client.get(reverse("event:post_bid" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0]["bid_info"]["bid_id"], 2)

    def test_get_invalid_bid(self):
        '''
        Getting bid with event_id 2, which does not exist
        '''
        response = self.client.get(reverse("event:post_bid" , args=[2]))
        self.assertEqual(response.status_code, 404)

    def test_post_valid_bid_list(self):
        '''
        Posting a  bid
        '''
        data = {
        ...
        self.assertEqual(response.data[0]["bid_info"]["bid_id"], 2)
        self.assertEqual(response.data[0]["bid_info"]["seller_id"], 3)
        self.assertEqual(response.data[0]["bid_item_list"][0]["item_info"]["bid_line_item_id"], 2)
        self.assertEqual(response.data[0]["bid_item_list"][0]["item_tax"][0]["tax_name"], "ll")
    
    def test_post_invalid_bid_list(self):
        '''
        Posting a new bid with invalid info
        '''
        data = {
            "bid_info":
            {
        ...
        response = self.client.post(reverse("event:post_bid", args=[2]), json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 400)    