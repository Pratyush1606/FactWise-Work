...
class BidViewTest(APITestCase):
    ...
    def test_get_valid_bid(self):
        '''
        Getting bid with ID 1
        '''
        response = self.client.get(reverse("event:get_bid" , args=[1]))
    ...
    def test_get_invalid_bid(self):
        '''
        Getting bid with ID 2, which does not exist
        '''
        response = self.client.get(reverse("event:get_bid" , args=[2]))
        self.assertEqual(response.status_code, 404)

    def test_patch_valid_bid(self):
        '''
        Patching bid with ID 1 with status
        '''
        data = {
            "status": "Response submitted"
        }
        data = json.dumps(data)
        response = self.client.patch(reverse("event:get_bid" , args=[1]), data, content_type='application/json')
        self.assertEqual(response.status_code, 200)
    
    def test_patch_invalid_bid(self):
        '''
        Patching bid with ID 2 with stauts
        '''
        data = {
            "status": "Response submitted"
        }
        data = json.dumps(data)
        response = self.client.patch(reverse("event:get_bid" , args=[2]), data, content_type='application/json')
        self.assertEqual(response.status_code, 404)
    
    def test_patch_deleted_bid(self):
        '''
        Patching deleted bid
        '''
        response = self.client.delete(reverse("event:get_bid" , args=[1]))
        data = {
            "status": "Response submitted"
        }
        data = json.dumps(data)
    ...
    def test_delete_valid_bid(self):
        # Deleting bid with ID 1
        response = self.client.delete(reverse("event:get_bid" , args=[1]))
        self.assertEqual(response.status_code, 200)
    
    def test_delete_invalid_bid(self):
        # Deleting bid with id 1
        response = self.client.delete(reverse("event:get_bid" , args=[1]))
        self.assertEqual(response.status_code, 200)
        # Again deleting bid with id 1
        response = self.client.delete(reverse("event:get_bid" , args=[1]))
        self.assertEqual(response.status_code, 400)