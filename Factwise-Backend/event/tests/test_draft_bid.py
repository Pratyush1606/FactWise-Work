...
class DraftBidViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db
        '''
        ...
        DraftBid.objects.create(**bid_info_data)
        self.draft_bid = DraftBid.objects.get(bid_id=1)
        draft_bid_item_data = {
            "bid_id" : self.draft_bid,
            "event_line_item_id" : self.event_item,
            "measurement_unit_id" : self.measurement_unit_id,
        ...
        DraftBidItem.objects.create(**draft_bid_item_data)
        self.draft_bid_item = DraftBidItem.objects.get(bid_line_item_id=1)
        draft_bid_item_tax_data = {
            "bid_line_item_id" : self.draft_bid_item,
            "tax_name" : "ll",
            "value" : 1
        }
        DraftBidItemTax.objects.create(**draft_bid_item_tax_data)
        

    def test_get_valid_draft_bid(self):
        '''
        Getting draft_bid with ID 1
        '''
        response = self.client.get(reverse("event:get_draft_bid" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["bid_info"]["bid_id"], 1)
        self.assertEqual(response.data["bid_item_list"][0]["item_info"]["bid_line_item_id"], 1)
        self.assertEqual(response.data["bid_item_list"][0]["item_tax"][0]["tax_name"], "ll")


    def test_get_invalid_draft_bid(self):
        '''
        Getting draft_bid with ID 2, which does not exist
        '''
        response = self.client.get(reverse("event:get_draft_bid" , args=[2]))
        self.assertEqual(response.status_code, 404)

    @patch.object(requests, "post")
    def test_patch_valid_draft_bid(self, mock_post):
        '''
        Patching draft_bid with ID 1 with name
        '''
        data = {
            "bid_info":
            {
                "seller_bid_id" : "123456",
                "status" : "Response Due"
            },
            "bid_item_list" :
            [
        ...
        mock_post.return_value.text = data_being_mocked
        data = json.dumps(data)
        response = self.client.patch(reverse("event:get_draft_bid" , args=[1]), data, content_type='application/json')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["bid_info"]["seller_bid_id"], "123456")
        self.assertEqual(response.data["bid_info"]["status"], "Response Due")
        self.assertEqual(response.data["bid_item_list"][0]["item_info"]["seller_comments"], "llhtu")

    @patch.object(requests, "post")
    def test_patch_invalid_draft_bid(self, mock_post):
        '''
        Patching draft_bid with ID 2 witch does not exist
        '''
        data = {
            "bid_info":
            {
        ...
        mock_post.return_value.text = data_being_mocked
        data = json.dumps(data)
        response = self.client.patch(reverse("event:get_draft_bid" , args=[2]), data, content_type='application/json')
        self.assertEqual(response.status_code, 404)