...
class DraftBidListViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db 
        '''
    ...
    def test_get_valid_draft_bid_list(self):
        '''
        Getting draft_bid_list with ID 1
        '''
        response = self.client.get(reverse("event:get_draft_bid_list" , args=[1,2]))
    ...
    def test_get_invalid_draft_bid_list(self):
        '''
        Getting draft_bid with ID 2, which does not exist
        '''
        response = self.client.get(reverse("event:get_draft_bid_list" , args=[2,2]))
        self.assertEqual(response.status_code, 404)

    def test_post_valid_draft_bid_list(self):
        '''
        Posting a draft bid
        '''
        data = {
        ...
        response = self.client.post(reverse("event:post_draft_bid_list", args=[1]), json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 201)
        self.assertEqual(response.data["bid_info"]["bid_id"], 2)
        self.assertEqual(response.data["bid_info"]["seller_id"], 3)
        self.assertEqual(response.data["bid_item_list"][0]["item_info"]["bid_line_item_id"], 2)
        self.assertEqual(response.data["bid_item_list"][0]["item_tax"][0]["tax_name"], "ll")
    
    def test_post_invalid_draft_bid_list(self):
        '''
        Posting a new draft_bid with invalid info
        '''
        data = {
            "bid_info":
            {
        ...
        '''
        Posting data valid for non exosting event
        '''
        data = {
            "bid_info":
            {
                "bid_creator_user_id" : 1, 
                "bid_creator_entity_type" : "B", 
                "buyer_id" : 1,
                "seller_id" : 2,
                "seller_bid_id" : "12345",
                "bid_creation_datetime" : "2021-08-02T06:03:08.583002Z",
        ...
        response = self.client.post(reverse("event:post_draft_bid_list", args=[2]), json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 404)