...
class DraftEventShiftEventViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db 
        Entering a draft_event_shift_event with event_id 1
        '''
    ...
    def test_post_valid_draft_event_shift_event(self):
        '''
        Posting draft_event_shift_event with ID 1
        '''
        response = self.client.post(reverse("event:draft_event_shift_event" , args=[1]))
        ...
        self.assertEqual(len(response.data["event_item_list"][0]["item_seller"]), 2)
        self.assertEqual(response.data["event_item_list"][0]["item_seller"][1]["seller_id"], 3)

    def test_post_invalid_draft_event_shift_event(self):
        '''
        Getting event_review_page with ID 2, which does not exist
        '''
        response = self.client.post(reverse("event:draft_event_shift_event" , args=[2]))
