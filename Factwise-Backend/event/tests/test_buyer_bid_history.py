...
class BuyerSellerBidHistoryViewTest(APITestCase):
    def setUp(self):
        '''
        Populating test db
        '''
    ...
    def test_get_valid_buyer_bid_history(self):
        '''
        Posting Bid for bid history
        '''
        data = {
            "bid_info":
            {
        ...
        '''
        Getting bid with event id 1
        '''
        response = self.client.get(reverse("event:buyer_bid_history" , args=[1]))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data[0][0]["bid_info"]["bid_id"], 1)
        self.assertEqual(response.data[0][0]["bid_info"]["seller_id"], 2)
        self.assertEqual(response.data[1][0]["bid_info"]["seller_id"], 3)
        self.assertEqual(response.data[0][0]["bid_item_list"][0]["item_tax"][0]["tax_name"], "ll")


    def test_get_invalid_buyer_bid_history(self):
        '''
        Getting bid with event id 1, which does not exist
        '''
        response = self.client.get(reverse("event:buyer_bid_history" , args=[1]))
        self.assertEqual(response.status_code, 404)
        '''
        Getting bid with event id 2, which does not exist
        '''
        response = self.client.get(reverse("event:buyer_bid_history" , args=[2]))
        self.assertEqual(response.status_code, 404)